...
ImgView::ImgView(QWidget *parent) {
  ...
  connect(this, &ImgView::customContextMenuRequested, this,
          &ImgView::customContextMenu);
  ...
  connect(btnSmooth, &QPushButton::clicked, [this, btnSmooth]() {
    ...
    btnSmooth->setText(m_antialiase ? QStringLiteral(u"🌀")
                                    : QStringLiteral(u"░"));
    QSettings settings("ImgView", "ImgView");
  ...
  connect(btnPrev, &QPushButton::clicked,
          [this]() { nextImage(FileDir::previous); });
  ...
  connect(btnWheelFunction, &QPushButton::clicked, [=, this]() {
    ...
    btnWheelFunction->setText(m_wheel_zoom ? QStringLiteral(u"🔍")
                                           : QStringLiteral(u"🔃"));
    QSettings settings("ImgView", "ImgView");
...
void ImgView::wheelEvent(QWheelEvent *event) {
  ...
  if (angle != 0) {
    ...
    if (ctrl == m_wheel_zoom) {
      ...
      QPointF const beforeScale =
          (mousePos - m_offset - QPointF(width() / 2., height() / 2.)) / m_zoom;
      m_zoom *= factor;
      m_zoom = std::max(0.01, std::min(10000., m_zoom));
      m_offset = mousePos - (beforeScale * m_zoom) -
...
void ImgView::getFiles(QStringList filenames, QDirIterator::IteratorFlag itf) {
  ...
  connect(dit, &DirIteratorTask::loadedFilenames, this,
          &ImgView::loadedFilenames);
...
void ImgView::openFolder(QString dir) {
  if (dir.isEmpty()) {
    ...
    dir = QFileDialog::getExistingDirectory(
        this, QStringLiteral(u"Open Folder"), lastDir);
  }
...
void ImgView::loaded(ImageInfo info, ImageItem *ii) {
  update();
...
void ImgView::nextImage(FileDir fd) {
  ...
  if (fd != FileDir::none) {
    int const nextidx = fitincircularrange(
        (fd == FileDir::next) ? m_mainImage->idx() + 1 : m_mainImage->idx() - 1,
        m_allImages.size());
    m_mainImage = m_allImages.at(nextidx);
    autofit();
  } else {
    update();
  }
...
void ImgView::setTransform() {
  ...
  QRectF const logicalRect =
      m_transform.inverted().mapRect(QRectF(this->rect()));
  for (auto &ii : m_allImages) {
...
void ImgView::customContextMenu(QPoint pos) {
  ...
  menu->addAction(QStringLiteral(u"Load Image"),
                  [this]() { loadImage(QStringList()); });
